...
Bacterial pneumonia needs to be treated with antibiotics, whereas viral pneumonia is treated with antivirals and other supportive care. 
Therefore it is important to detect pneumonia in an early state to combat the bacteria or virus. 
Because of the fact that newborns often don't show any signs of the disease the possibility of detecting Pneumonia 
by imaging methods such as X-ray or CT is an important medical method. 
Additionally, X-ray is a standard treatment and can help differentiate between the different cause for the infection [1-5].
...
The data set is used in a lot of algorithms found on kaggle. The approaches are very different, 
often they use CNN as network, but some also use transfer learning. 
...
The reason will be explained in chapter \glqq{}Neural Network from scratch\grqq{}.
...
Either the X-ray shows no signs of an infection (\glqq{}NORMAL\grqq{}) or one can see Pneumonia 
developing in the lungs (\glqq{}PNEUMONIA\grqq{}). In the code we represent this binary classification as numbers:

\begin{itemize}
  \item \glqq{}NORMAL\grqq{} : 0
  \item \glqq{}PNEUMONIA\grqq{} : 1
\end{itemize}
...
The model uses the \glqq{}Adam\grqq{} optimizer and the loss function is the binary cross-entropy one.
...
We measured the performance of the model by accuracy, prescision and recall. 
...
We learned that overfitting can be treated by using regularization, by smaller network and by more data. 
...
\section{References}
\begin{enumerate}
  \item https://www.lungeninformationsdienst.de/krankheiten/lungenentzuendung/verbreitung
  \item https://www.kaggle.com/datasets/tolgadincer/labeled-chest-xray-images
  \item https://data.mendeley.com/datasets/rscbjbr9sj/2
  \item https://www.kaggle.com/datasets/paultimothymooney/chest-xray-pneumonia
  \item https://www.cell.com/cell/fulltext/S0092-8674(18)30154-5
\end{enumerate}
